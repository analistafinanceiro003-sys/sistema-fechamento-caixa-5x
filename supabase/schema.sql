...
-- ============================================================
-- PERFIL ANALISTA (multiempresa)
-- Mesmo acesso de edição do Master, exceto a aba Sistema (configurações
-- gerais, fornecedores/categorias em escrita, backup, logs de auditoria,
-- documentos). Diferente de Admin/Operador, o Analista não é vinculado a
-- uma única empresa: o Master escolhe um conjunto de empresas (tabela
-- analyst_companies) ao criar o acesso, pela aba Sistema → Analistas.
-- Bloco seguro para re-executar.
-- ============================================================
...
-- ============================================================
-- DIFERENÇA DE REPASSE — ACEITAR/ENCERRAR (transfer_waivers)
-- Quando o repasse informado por um fechamento fica abaixo do esperado
-- (fundo padrão da loja) e o cliente nunca chega a confirmar o repasse,
-- a diferença ficava "pendente" para sempre em Resumo por Fechamento e
-- em Repasses. Esta tabela guarda o aceite/encerramento dessa diferença
-- pelo Master (com justificativa obrigatória) — não apaga nem finge que
-- o valor foi recebido, só registra que foi revisado e aceito como está.
-- Bloco seguro para re-executar.
-- ============================================================
...
-- ============================================================
-- COORDENADORES (múltiplos acessos Master)
...
--
-- insert into public.profiles (user_id, name, email, role, status, is_owner)
-- values (
...
-- );
--
-- Demais acessos Master (coordenadores) são criados pela tela Sistema →
-- Coordenadores (só aparece/funciona para quem já é is_owner=true) — não
-- precisa de SQL manual para esses.
--
...
-- );
--
-- PARA CRIAR ANALISTA (normalmente feito pela tela Sistema → Analistas,
-- que já grava os vínculos de empresa automaticamente; abaixo é o
-- equivalente manual caso precise via SQL):
-- insert into public.profiles (user_id, name, email, role, status)
-- values (
--   '<UUID_DO_ANALISTA>',
--   'Nome do Analista',
--   'analista@gestao5x.com.br',
--   'analyst',
--   'Ativo'
-- )
-- returning id;
-- -- em seguida, para cada empresa liberada:
-- insert into public.analyst_companies (profile_id, company_id)
-- values ('<ID_DO_PROFILE_ACIMA>', '<UUID_DA_EMPRESA>');
